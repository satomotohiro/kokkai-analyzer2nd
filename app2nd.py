...
if st.button("📡 検索して分析"):
   ...
   # 要約プロンプト（議員か政党かで分岐）
    ...
    ・「{keyword}」に関して{selected_politician}が述べた内容を要約し、20字以内の見出しとして出力してください（例：「防衛費の増額を支持」など）。
    ...
    ・「{keyword}」に関して{selected_party}の立場を要約し、20字以内の見出しとして出力してください（例：「消費税減税に慎重姿勢」など）。

...
if st.button("📡 検索して分析"):
    ...
    if selected_politician != "指定しない":
        ...
        ・「{'、'.join(keywords)}」に関して{selected_politician}が述べた内容を要約し、20字以内の見出しとして出力してください（出力例：「防衛費の増額を支持」などとしてください。「見出し：」のような書き出しは不要です。）。
        ...
        次に、以下2つの出力をそれぞれ順番に提供してください：
        ・「{'、'.join(keywords)}」に関して{selected_party}の立場を要約し、20字以内の見出しとして出力してください（出力例：「消費税減税に慎重姿勢」などとしてください。「見出し：」のような書き出しは不要です。）。

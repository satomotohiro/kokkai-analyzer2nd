...
if st.button("📡 検索して分析"):
    ...
    # --- Gemini 要約プロンプト（文脈と立場の識別付き） ---
    prompt = f"""
    以下は日本の国会における一人または複数の議員の発言記録です。
    それぞれの発言が「質問」なのか「答弁（内閣側など政策説明を含む）」なのかをまず判別してください。
    
    そのうえで、発言内容から「{keyword}」に関して、答弁者の立場や政策的姿勢を重視し、どのような考え方を持っているかを200字以内で簡潔にまとめてください。
    一人の議員の発言のみの場合はその議員の考え方として、「{selected_politician}は」で書き出してください。
    複数の議員の発言の場合はその議員らが属する政党の見解として、「{selected_party}は」で書き出してください。
    
    ※ 注意点：
    - 質問者の発言は、相手の立場を引き出すためのものなので、誤ってそのまま見解と解釈しないでください。
    - 文脈をよく読み、発言者が本当に主張している内容を理解してください。

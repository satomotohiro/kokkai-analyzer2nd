...
if st.button("📡 検索して分析"):
   ...
   # 要約プロンプト（議員か政党かで分岐）
    ...
    1. 「{keyword}」に関して{selected_politician}が述べた内容を要約し、20字以内の見出しとして出力してください（例：「防衛費の増額を支持」など）。
    2.  そのうえで「{keyword}」に関して、{selected_politician}がどのような立場や政策的考えを持っているかを、文脈を踏まえて**200字以内**で要約してください。（出力は「{selected_politician}は〜」で始めてください）。
    ...
    1. 「{keyword}」に関して{selected_party}の立場を要約し、20字以内の見出しとして出力してください（例：「消費税減税に慎重姿勢」など）。
    2. そのうえで「{keyword}」に関して、{selected_party}がどのような政策的立場・思想を持っているかを、文脈を踏まえて**200字以内**で要約してください。（出力は「{selected_party}は〜」で始めてください）。
